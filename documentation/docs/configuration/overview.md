...
├── synced_schedule.toml   # Accepted daily task overlay from `reminder sync`
└── tasks/
...
### 4. Daily Sync Overlay (`synced_schedule.toml`)
An optional TOML overlay generated by `reminder sync`. It stores the accepted
...
`reminder sync` generates the same custom-title form for accepted task assignments, for example:
